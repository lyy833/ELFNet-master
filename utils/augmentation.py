...
class TrendSeasonalAugmenter:
    ...
    def _detrend_reshuffle_trend(self, x):
        """去趋势、重排趋势再复原"""
        T = x.size(0)
        ...
        detrended = x - linear_trend  # 残差，保持不变
        
        # 2. 对趋势序列进行分段重排
        # 确定分段长度，确保至少能分2段
        min_segment_size = 4
        max_segments = max(2, T // min_segment_size)
        segment_size = max(min_segment_size, T // max_segments)
        
        # 将趋势序列分段
        trend_segments = []
        for i in range(0, T, segment_size):
            segment = linear_trend[i:i+segment_size]
            if len(segment) > 0:
                trend_segments.append(segment)
        
        # 随机重排趋势片段
        indices = torch.randperm(len(trend_segments))
        shuffled_trend_segments = [trend_segments[i] for i in indices]
        
        # 重新拼接趋势
        reshuffled_trend = torch.cat(shuffled_trend_segments)
        
        # 长度处理：确保重排后趋势长度与原序列一致
        if len(reshuffled_trend) < T:
            # 用最后一个值填充
            padding = torch.full((T - len(reshuffled_trend),), 
                                reshuffled_trend[-1], device=x.device)
            reshuffled_trend = torch.cat([reshuffled_trend, padding])
        elif len(reshuffled_trend) > T:
            reshuffled_trend = reshuffled_trend[:T]
        
        # 3. 将重排后的趋势与原始残差相加
        x_neg = reshuffled_trend + detrended
    ...
    def _seasonal_scale(self, x):
        """
        多模式季节性缩放破坏 - 随机选择不同破坏策略增强负样本多样性
        缩放破坏模式：1)强衰减 2)强放大 3)符号反转 4)混合破坏
        """
        T = x.shape[0]
        
        # 1. 获取有效周期并提取季节性成分
        effective_period = self._get_effective_period(T)
        seasonal_component, non_seasonal_component = self._extract_robust_seasonal_component(x, effective_period)
        
        # 2. 计算季节性显著性
        seasonal_significance = self._calculate_seasonal_significance(seasonal_component, x)
        
        # 3. 根据显著性选择破坏策略池
        if seasonal_significance < 0.3:  # 弱季节性：使用相对简单且温和的破坏策略
            strategies = ['attenuate_m', 'amplify_m']  # 中等衰减、中等放大
        else:  # 强季节性：使用更复杂强烈的破坏策略
            strategies = ['attenuate', 'amplify', 'invert', 'hybrid'] # 强衰减、强放大、部分/完全反转、混合破坏
        
        # 4. 随机选择破坏模式
        strategy = np.random.choice(strategies)
        
        # 5. 应用选择的破坏策略
        if strategy == 'attenuate_m':  
            scale_factor = 0.3 + 0.2 * torch.rand(1).to(x.device)  # 0.3-0.5 (中等衰减)

        elif strategy == 'amplify_m':  
            scale_factor = 1.8 + 0.6 * torch.rand(1).to(x.device)  # 1.8-2.4 (中等放大)

        elif strategy == 'attenuate':  
            scale_factor = 0.1 + 0.2 * torch.rand(1).to(x.device)  # 0.1-0.3 (强衰减)

        elif strategy == 'amplify': 
            scale_factor = 2.4 + 0.6 * torch.rand(1).to(x.device)  # 2.4-3.0 (强放大)
                           
        elif strategy == 'invert':  # 符号反转
            # 两种反转模式：随机选择
            if torch.rand(1).item() < 0.5:
                scale_factor = -0.5 - 0.3 * torch.rand(1).to(x.device)  # -0.8 - -0.5 (部分反转)
            else:
                scale_factor = -1.5 - 0.5 * torch.rand(1).to(x.device)  # -2.0 - -1.5 (完全反转)
                
        else:  # 'hybrid' - 混合破坏（衰减+反转）
            # 先衰减再部分反转，创造更复杂的破坏模式
            attenuation = 0.3 + 0.2 * torch.rand(1).to(x.device)  # 0.3-0.5
            inversion = -0.5 - 0.3 * torch.rand(1).to(x.device)   # -0.8 - -0.5
            scale_factor = attenuation * inversion  # 最终为负值，幅度在0.24-0.4之间
        
        # 6. 应用缩放
        scaled_seasonal = seasonal_component * scale_factor
        
        # 7. 重组序列
        x_neg = scaled_seasonal + non_seasonal_component
        
        return x_neg

    def _extract_robust_seasonal_component(self, x, period):
        """
        鲁棒的季节性成分提取方法
        结合了均值法和移动平滑，减少噪声影响
        """
        ...
        if period < 2 or period >= T:
            # 无法提取，返回零季节性成分
            seasonal_component = torch.zeros_like(x)
            non_seasonal_component = x.clone()
            return seasonal_component, non_seasonal_component
        
        # 方法1：简单的周期性均值法
        seasonal_pattern = torch.zeros(period, device=x.device)
        count_pattern = torch.zeros(period, device=x.device)
        
        for t in range(T):
            phase = t % period
            seasonal_pattern[phase] += x[t]
            count_pattern[phase] += 1
        
        count_pattern = torch.clamp(count_pattern, min=1)
        seasonal_pattern = seasonal_pattern / count_pattern
        
        # 扩展季节性模式
        seasonal_component = torch.zeros_like(x)
        for t in range(T):
            seasonal_component[t] = seasonal_pattern[t % period]
        
        # 方法2：使用移动平均平滑季节性成分
        # 这可以进一步减少噪声，但可能平滑掉一些细节
        if period >= 3:
            # 对季节性成分进行轻度平滑
            kernel_size = min(3, period // 2)
            weights = torch.ones(kernel_size, device=x.device) / kernel_size
            seasonal_component = torch.conv1d(
                seasonal_component.unsqueeze(0).unsqueeze(0),
                weights.unsqueeze(0).unsqueeze(0),
                padding=kernel_size // 2
            ).squeeze()
        
        non_seasonal_component = x - seasonal_component
        
        return seasonal_component, non_seasonal_component

    def _calculate_seasonal_significance(self, seasonal_component, original_x):
        """
        计算季节性成分的显著性
        基于季节性成分能解释的方差比例
        """
        seasonal_variance = seasonal_component.var()
        total_variance = original_x.var()
        
        if total_variance == 0:
            return 0.0
        
        # 计算解释的方差比例
        explained_variance = seasonal_variance / total_variance
        
        # 限制在[0, 1]范围内
        significance = torch.clamp(explained_variance, 0.0, 1.0).item()
    ...
    def _get_effective_period(self, T):
        ...
        if T < base_period:  
            # 优先尝试base_period的约数
            for divisor in [base_period // 2, base_period // 3, base_period // 4]:
...
class DomainAugmentationFramework:
    ...
    def augment_batch(self, batch_x, plot_dir=None, plot_augment_flag=False):
        """批处理增强 - 各司其职"""
        # 1. 正负增强：因果感知增强（核心）
