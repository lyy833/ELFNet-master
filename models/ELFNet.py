...
class MixedChannelConvEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        for idx, group in enumerate(self.groups):
            ...
            group_features = group_features.transpose(1, 2).to(x.device)
...
class BandedFourierLayer(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, band, num_bands, length=201):
        ...
        self.band = band  # zero indexed 频率带索引，在MultiBandSeasonalDisentangler调用这个部件时传入的band从0到num_bands-1
        self.num_bands = num_bands ### 总的频带数量，默认为3

        # 前 num_bands-1 个频带每个获得 total_freqs // num_bands 个频率分量，最后一个频带获得剩余的所有分量：total_freqs // num_bands + (total_freqs % num_bands)
        self.num_freqs = self.total_freqs // self.num_bands + (self.total_freqs % self.num_bands if self.band == self.num_bands - 1 else 0)

        self.start = self.band * (self.total_freqs // self.num_bands) # 当前频带起始频率索引
        self.end = self.start + self.num_freqs  # 当前频带结束频率索引
...
class ELFNet_supervised(nn.Module): 
    def __init__(self,args, device):
        """
        有监督版ELFNet
        """
        super(ELFNet_supervised, self).__init__()
