...
class CustomDataset(Dataset):
    ...
    def __read_data__(self):
        ...
        # 4. 根据模式确定数据边界
        if self.pretrain_stage and self.args.training_mode == 'one2many':
        ...
        if self.scale:
            if self.pretrain_stage and self.args.training_mode == 'one2many':
                ...
                train_border1 = 0 if (self.pretrain_stage and self.args.training_mode == 'one2many') else border1s[0]
                train_border2 = len(df_data) if (self.pretrain_stage and self.args.training_mode == 'one2many') else border2s[0]
    ...
    def inverse_transform(self, data,target_idx):
        """
        将标准化后的目标变量数据反标准化为原始数据
        
        Args:
            data: 标准化后的目标变量数据，形状是 [batch_size, pred_len, 1]
        Returns:
            反标准化后的目标变量数据，形状与输入相同
        """

        # 获取目标变量的均值和标准差
        target_mean = self.scaler.mean_[target_idx]
        target_std = self.scaler.scale_[target_idx]  # scale_存储的是标准差
        
        # 确保target_std不为0，避免除零
        if target_std == 0:
            target_std = 1.0
        ...
        # 反标准化: original = normalized * std + mean
        denormalized_flat = data_flat * target_std + target_mean
        # 恢复原始形状
