...
def plot_decomposition_df(dataset,df, column): 
    if dataset == 'XJ_Photovoltaic.csv':  # 15 minutes
        df = df.resample('D').mean() #重采样——1D
        period = 96  # 24 hours * 4 periods 
    elif dataset ==  'Australia_Load&Price.csv':  # 30 minutes
        df = df.resample('W').mean() #重采样——1W
        period = 24   # 24 hours
    elif dataset == 'Mathematical_Modeling_Competition.csv':  # 1 day
        period = 30  # Approximate days in a month
    elif dataset == 'Panama_CND':  # 1 hour
        df = df.resample('W').mean() #重采样——1W
        period = 24   # 24 hours
    else:
    ...
    # 趋势成分
    plt.subplot(4, 1, 2)
    plt.plot(decomposition.trend, label='趋势成分', color='orange')
    ...
    # 季节性成分
    plt.subplot(4, 1, 3)
    plt.plot(decomposition.seasonal, label='季节成分', color='green')
    ...
    # 残差成分
    plt.subplot(4, 1, 4)
    plt.plot(decomposition.resid, label='残差成分', color='red')
...
if __name__ == '__main__':
    ...
    folder_path = r'C:/Users/think/Downloads/ELFNet-main/datasets/'
    dataset = 'Australia_Load&Price.csv'
    ...
    task = 'resample_plot' # ['raw_plot', 'resample_plot',decomposition_plot]
    ...
    if task == 'raw_plot':
        if dataset == 'Mathematical_Modeling_Competition.csv':
        ...
        elif dataset == 'Australia_Load&Price.csv':
            ...
            plot_raw_df(raw_data, x='date', y_columns=['load'])
        elif dataset == 'XJ_Photovoltaic.csv':
        ...
        elif dataset == 'XJ_Photovoltaic_one_week_load.csv':
            plot_raw_df(raw_data, x='date', y_columns=['load(mw)'])
        else:
        ...
        elif dataset == '2019_Xinjiang_Photovoltaic_Data.csv'or dataset == 'Australia_Load&Price.csv':
            for column in raw_data.columns:
